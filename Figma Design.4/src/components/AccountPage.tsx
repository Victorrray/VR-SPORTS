    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {onNavigateToSettings && (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 ${isLight ? 'bg-gradient-to-r from-purple-100 to-indigo-100 border-purple-300 text-purple-700' : 'bg-gradient-to-r from-purple-500/20 to-indigo-500/20 border-purple-400/30 text-purple-300'} backdrop-blur-xl border rounded-xl ${isLight ? 'hover:from-purple-200 hover:to-indigo-200' : 'hover:from-purple-500/30 hover:to-indigo-500/30'} transition-all font-bold shadow-lg ${isLight ? '' : 'shadow-purple-500/20'} text-sm whitespace-nowrap self-start sm:self-auto`}
          >
      ...
      <div className={`${isLight ? lightModeColors.statsCard : 'bg-gradient-to-br from-white/5 via-white/[0.02] to-transparent border-white/10'} backdrop-blur-2xl border rounded-2xl p-6 shadow-xl`}>
        <div className="flex items-center justify-between mb-6">
          ...
          <button className={`px-4 py-2 ${isLight ? 'bg-white border-gray-300 text-gray-700 hover:bg-gray-50' : 'bg-white/5 border-white/10 text-white hover:bg-white/10'} backdrop-blur-xl border rounded-xl transition-all font-bold text-sm`}>
            Edit Profile
          </button>
        </div>

        <div className="flex items-center gap-6 mb-6">
          <div className={`w-20 h-20 rounded-2xl ${isLight ? 'bg-purple-100 border-purple-200' : 'bg-gradient-to-br from-purple-500/30 to-indigo-500/30 border-purple-400/30'} border flex items-center justify-center backdrop-blur-xl shadow-lg ${isLight ? '' : 'shadow-purple-500/20'}`}>
      ...
      <div className={`${isLight ? lightModeColors.statsCard : 'bg-gradient-to-br from-white/5 via-white/[0.02] to-transparent border-white/10'} backdrop-blur-2xl border rounded-2xl p-6 shadow-xl`}>
        ...
        <div className={`p-6 ${isLight ? 'bg-amber-50 border-amber-200' : 'bg-gradient-to-br from-amber-500/10 via-orange-500/10 to-transparent border-amber-400/30'} backdrop-blur-xl border rounded-xl shadow-lg ${isLight ? '' : 'shadow-amber-500/10'}`}>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <div className={`p-3 ${isLight ? 'bg-amber-100 border-amber-200' : 'bg-gradient-to-br from-amber-500/30 to-orange-500/30 border-amber-400/30'} rounded-xl border shadow-lg ${isLight ? '' : 'shadow-amber-500/20'}`}>
      ...
      <div className={`${isLight ? lightModeColors.statsCard : 'bg-gradient-to-br from-white/5 via-white/[0.02] to-transparent border-white/10'} backdrop-blur-2xl border rounded-2xl p-6 shadow-xl`}>
        ...
        <div className="space-y-4">
          <div className={`flex items-center justify-between p-4 ${isLight ? 'bg-gray-50 border-gray-200' : 'bg-gradient-to-br from-white/5 to-transparent border-white/10'} backdrop-blur-xl rounded-xl border`}>
            <div className="flex items-center gap-3">
              <Bell className={`w-5 h-5 ${isLight ? 'text-purple-600' : 'text-purple-400'}`} />
              <div>
                <div className={`${isLight ? 'text-gray-900' : 'text-white'} font-bold`}>
                  Email Notifications
                </div>
                <div className={`${isLight ? 'text-gray-500' : 'text-white/50'} text-sm font-bold`}>
                  Receive updates on new picks
                </div>
              </div>
            </div>
      ...
      <div className={`${isLight ? lightModeColors.statsCard : 'bg-gradient-to-br from-white/5 via-white/[0.02] to-transparent border-white/10'} backdrop-blur-2xl border rounded-[16px] p-6 shadow-xl`}>
        <h2 className={`${isLight ? 'text-gray-900' : 'text-white'} font-bold flex items-center gap-2 mb-6`}>
          <Shield className={`w-5 h-5 ${isLight ? 'text-purple-600' : 'text-purple-400'}`} />
          Security & Privacy
        </h2>
      ...
      {/* Danger Zone */}
      <div className={`${isLight ? 'bg-red-50 border-red-200' : 'bg-gradient-to-br from-red-500/10 via-red-500/5 to-transparent border-red-400/30'} backdrop-blur-2xl border rounded-2xl p-6 shadow-xl ${isLight ? '' : 'shadow-red-500/10'}`}>
