        ...
        {showActions && (
          <div className={`grid ${isHero ? 'grid-cols-1' : 'grid-cols-2'} gap-2`}>
            {!isHero && (
              ...
              } backdrop-blur-xl border rounded-md transition-all font-bold text-xs text-center`}>
                Compare Odds
              </button>
            )}
            {!isHero && (
              <button 
                ...
                } text-white rounded-full transition-all font-bold text-xs border text-center flex items-center justify-center gap-1.5`}
              >
                {isAdded ? (
                  <>
