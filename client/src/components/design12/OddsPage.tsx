...
export function OddsPage({ onAddPick, savedPicks = [], betType, onBetTypeChange }: OddsPageProps) {
  ...
  const PaywallOverlay = () => (
    <div className="min-h-[60vh] flex items-center justify-center px-4">
      <div className={`max-w-lg w-full text-center ${isLight ? 'bg-white border-gray-200' : 'bg-slate-900/90 border-white/10'} border rounded-3xl p-8 md:p-12 backdrop-blur-xl shadow-2xl`}>
        {/* Lock Icon */}
        <div className={`w-20 h-20 mx-auto mb-6 rounded-2xl ${isLight ? 'bg-purple-100 border-purple-200' : 'bg-purple-500/30 border-purple-400/30'} border flex items-center justify-center`}>
          ...
          >
            ...
            <div className={`sticky top-0 ${isLight ? 'bg-white' : 'bg-slate-950'} z-10 px-5 pt-5 lg:pt-5 pb-3 space-y-3 lg:border-b border-b-0 ${isLight ? 'border-gray-100' : 'border-white/5'} -mt-6 lg:mt-0 lg:rounded-none max-lg:rounded-t-2xl`}>
              ...
              <div className="flex lg:hidden gap-2">
                <button
                  ...
                  className="flex-1 px-4 py-2.5 rounded-full text-sm transition-colors text-center bg-purple-600 text-white hover:bg-purple-500"
                >
                  Apply Filters
                </button>
                <button
                  ...
                  className={`px-4 py-2.5 rounded-full text-sm transition-colors text-center ${
                    isLight 
            ...
            <div className="overflow-y-auto flex-1 p-5 pt-4 lg:pt-6 space-y-4 scrollbar-hide">
              ...
              <div className="relative">
                ...
                {dateExpanded && (
                  <div className={`hidden lg:block mt-1.5 ${isLight ? 'bg-white border-gray-200 shadow-lg' : 'bg-slate-900 border-white/10'} border rounded-2xl overflow-hidden`}>
              ...
              {dateExpanded && createPortal(
                <>
                  ...
                  {/* Bottom Drawer */}
                  <div className={`lg:hidden fixed bottom-0 left-0 right-0 max-h-[60vh] ${isLight ? 'bg-white' : 'bg-slate-900'} rounded-t-3xl z-[10001] overflow-hidden ${dateClosing ? 'animate-out slide-out-to-bottom' : 'animate-in slide-in-from-bottom'} duration-300`}>
                    {/* Drag Handle */}
              ...
              {sportsbooksExpanded && createPortal(
                <>
                  {/* Backdrop */}
                  <div 
                    className={`lg:hidden fixed inset-0 bg-black/80 backdrop-blur-md z-[10000] transition-opacity duration-300 ${sportsbooksClosing ? 'opacity-0' : 'opacity-100'}`}
                    onClick={closeSportsbooksDrawer}
                  />
                  
                  {/* Bottom Drawer */}
                  <div className={`lg:hidden fixed bottom-0 left-0 right-0 max-h-[70vh] ${isLight ? 'bg-white' : 'bg-slate-900'} rounded-t-3xl z-[10001] overflow-hidden ${sportsbooksClosing ? 'animate-out slide-out-to-bottom' : 'animate-in slide-in-from-bottom'} duration-300`}>
                    {/* Drag Handle */}
                    <div className="flex justify-center pt-3 pb-2">
                      <div className={`w-12 h-1.5 rounded-full ${isLight ? 'bg-gray-300' : 'bg-white/20'}`}></div>
                    </div>
              ...
              {marketExpanded && createPortal(
                <>
                  {/* Backdrop */}
                  <div 
                    className={`lg:hidden fixed inset-0 bg-black/80 backdrop-blur-md z-[10000] transition-opacity duration-300 ${marketClosing ? 'opacity-0' : 'opacity-100'}`}
                    onClick={closeMarketDrawer}
                  />
                  
                  {/* Bottom Drawer */}
                  <div className={`lg:hidden fixed bottom-0 left-0 right-0 max-h-[60vh] ${isLight ? 'bg-white' : 'bg-slate-900'} rounded-t-3xl z-[10001] overflow-hidden ${marketClosing ? 'animate-out slide-out-to-bottom' : 'animate-in slide-in-from-bottom'} duration-300`}>
                    {/* Drag Handle */}
                    <div className="flex justify-center pt-3 pb-2">
                      <div className={`w-12 h-1.5 rounded-full ${isLight ? 'bg-gray-300' : 'bg-white/20'}`}></div>
                    </div>
